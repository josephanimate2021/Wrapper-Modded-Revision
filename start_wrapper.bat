...
if !AUTOUPDATE!==y ( 
	...
	if exist .git (
		echo Updating...
		call utilities\PortableGit\bin\git.exe pull || echo Updating Wrapper Has Failed! Attempting To Reset On Whatever went wrong. && call utilities\PortableGit\bin\git.exe stash && call utilities\PortableGit\bin\git.exe pull || echo Reseting The Updater has failed! This is an auto updater. so you will just have to do this in an seperete window while wrapper keeps on starting up. && start update_wrapper.bat && start "" /wait /B "%~F0" point_insertion
...
if not exist utilities\config.bat ( echo Something is horribly wrong. You may be in a read-only system/admin folder. & pause & exit ) else ( call utilities\config.bat )
goto restart
:configavailable
...
if !INCLUDEDCHROMIUM!==n (
	...
	echo Opening Wrapper: Offline using %BROWSER%...
...
if !INCLUDEDCHROMIUM!==n (
	if !CUSTOMBROWSER!==n (
        ...
        if !USINGINCLUDEDCHROMIUM!==n ( set BROWSER=Google Chrome Version 68 ) else ( set BROWSER=included Chromium )
	echo Opening the server page using %BROWSER%...
        if !USINGINCLUDEDCHROMIUM!==n (
        pushd utilities\chrome-68
	if !APPCHROMIUM!==y (
...
:configcopy
cd %USERPROFILE%\Downloads\Wrapper-Modded-Revision
if not exist utilities ( md utilities )
copy temps\config.bat utilities
